...
try:
    ...
    def export_metrics_pdf(
        ...
        out_dir="outputs/metrics/ica-iclabel-autoreject",
    ...
    # Carregar o registo público PN00-4. O ficheiro não faz parte do
    ...
    raw_corrigido = ica.apply(raw_baseline.copy(), exclude=exclude_idx)

    # Dividir o sinal em épocas.
    dur = find_epoch_duration_that_tiles_raw(
        raw_corrigido, target_sec=2.0, min_sec=1.0, max_sec=3.0
    )
    ...
    ar, meta = configure_autoreject_auto(raw_corrigido, random_state=42, epoch_len=dur)
    print(meta)
    ar.fit(epochs)
    ...
    raw_corrigido_final = reconstruir_raw_de_epochs(epochs_out)
    raw_corrigido_final.set_montage(raw.get_montage(), on_missing="ignore")

    left_chs, right_chs = symmetric_temporal_pairs(raw)

    # Intervalos pré-ictal e ictal do registo PN00-4.
    results_before = compute_bsi_pre_ictal(
        raw, left_chs, right_chs, 0, 1005, 1006, 1080
    )

    results_after = compute_bsi_pre_ictal(
        raw_corrigido_final, left_chs, right_chs, 0, 1005, 1006, 1080
    )
    ...
    export_metrics_pdf(rows, "ICA+ICLabel+Ar_PN00-4.pdf")

    raw.plot(title="Sinal Padronizado")
    raw_corrigido_final.plot(title="Sinal com ICA+ICLabel+AutoReject")
    input("Pressione Enter para continuar...")

    os.makedirs("outputs/signals/ica-iclabel-autoreject", exist_ok=True)
    raw_corrigido_final.save(
        "outputs/signals/ica-iclabel-autoreject/PN00-4_corrigido.fif", overwrite=True
    )
