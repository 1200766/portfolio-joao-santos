...
try:
    ...
    def adapt_grid_with_dirtiness(Q, n_interpolate, consensus, pct_extreme):
        ...
        """
        # Classificar a proporção de épocas extremas.
        if pct_extreme < 0.05:
            level = "clean"
        ...
        # Limitar o número de canais interpolados em função do total.
        ...
        if level == "clean":
            # Reduzir a agressividade e usar a metade inferior da grelha.
            ...
            # Alargar a grelha para permitir a reparação de mais canais.
            extra = []
            for frac in [0.25, 0.35]:
                extra.append(min(cap_by_Q, max(1, int(round(frac * Q)))))
            n_int = sorted(set(n_int + extra))
            # Incluir um consenso mais elevado para reduzir a rejeição de épocas.
    ...
    def find_epoch_duration_that_tiles_raw(
        ...
        if not candidates:
            raise ValueError(
                "Não foi encontrada uma duração de época que divida "
                f"{n_times} amostras entre {min_sec} e {max_sec} segundos."
            )
    ...
    def reconstruir_raw_de_epochs(epochs_corrigidos):
        ...
        # Criar a informação correspondente.
    ...
    def metrics_to_rows(metrics_out, signal_name="Sinal"):
        ...
        # Métricas oculares: quatro rácios antes e quatro depois.
        ...
        # Calcular a diferença entre os valores antes e depois.
        for r in rows:
    ...
    def export_metrics_pdf(
        ...
        out_dir="outputs/metrics/autoreject-ica-iclabel",
    ...
    def symmetric_temporal_pairs(
        ...
        # Ordenar por lateralidade, da maior para a menor coordenada |x|.
    ...
    def normalize_1020_eeg_name(ch_name: str) -> str:
        ...
        # Remover o prefixo habitual.
        if name.upper().startswith("EEG "):
            name = name[4:].strip()

        # Aplicar as normalizações específicas do conjunto de dados.
    ...
    def muscle_metrics_temporal(
        ...
        def compute_on_raw(raw):
            ...
            for ch in temporal:
                ...
                # Declive 1/f por ajuste no domínio log-log.
    ...
    # Carregar o registo público PN00-2. O ficheiro não faz parte do
    # repositório; ver scripts/download_siena_pn00.py.
    file_path = "data/PN00-2.edf"
    raw = mne.io.read_raw_edf(file_path, preload=True, verbose=True)

    # Manter apenas os canais EEG e normalizar os respetivos nomes.
    ...
    # Configurar e ajustar o AutoReject uma vez.
    ar, meta = configure_autoreject_auto(raw_baseline, random_state=42, epoch_len=dur)
    print(meta)
    ar.fit(epochs)

    # Identificar as épocas rejeitadas.
    reject_log = ar.get_reject_log(epochs)
    bad_idx = np.where(reject_log.bad_epochs)[0]
    good_idx = np.where(~reject_log.bad_epochs)[0]

    # Reparar as épocas aceites.
    epochs_repaired = ar.transform(epochs.copy())

    # Confirmar a correspondência entre as épocas reparadas e as aceites.
    if len(epochs_repaired) != len(good_idx):
        raise RuntimeError(
            f"AutoReject transform devolveu {len(epochs_repaired)} epochs, "
            f"mas esperava {len(good_idx)} (épocas aceites). "
            "A quantidade ou a ordem das épocas não corresponde ao esperado."
        )

    # Preservar o número de épocas do sinal original.
    ...
    assert_channel_positions(raw_corrigido)
    ...
    ica.fit(raw_corrigido.copy())

    proba = iclabel_label_components(raw_corrigido, ica, inplace=True)
    exclude_idx, reasons = choose_exclude_multilabel_conservative(
        proba, thr=THRESH_TRAIN_ACC
    )

    print(
        f"ICs a excluir (multi-label conservador): {len(exclude_idx)} "
        f"/ {ica.n_components_}"
    )
    ...
    raw_corrigido_final = ica.apply(raw_corrigido.copy(), exclude=exclude_idx)
    left_chs, right_chs = symmetric_temporal_pairs(raw)

    # Intervalos pré-ictal e ictal do registo PN00-2.
    results_before = compute_bsi_pre_ictal(
        raw, left_chs, right_chs, 0, 1219, 1220, 1274
    )

    results_after = compute_bsi_pre_ictal(
        raw_corrigido_final, left_chs, right_chs, 0, 1219, 1220, 1274
    )

    print("Antes:", results_before)
    print("Depois:", results_after)

    out = avaliar_metricas_3_grupos(raw, raw_corrigido_final)
    rows = metrics_to_rows(out, signal_name="PN00-2")
    export_metrics_pdf(rows, "AR+ICA+ICLabel_PN00-2.pdf")

    raw.plot(title="Sinal Original")
    raw_corrigido_final.plot(title="Sinal com AutoReject")
    input("Pressione Enter para continuar...")

    os.makedirs("outputs/signals/autoreject-ica-iclabel", exist_ok=True)
    raw_corrigido_final.save(
        "outputs/signals/autoreject-ica-iclabel/PN00-2_corrigido.fif", overwrite=True
    )

finally:
    # -------------------------------------------------
    # Fim da medição do tempo, incluindo execuções com erro.
    # -------------------------------------------------
